export default {
  ...
  home: {
    ...
    features: {
      ...
      multiAccount: '稳如中投',
      multiAccountDesc: '多账号调度、失败重试、自动切换，关键时刻不掉线。',
      balanceQuota: '只因用量清楚',
      balanceQuotaDesc: '额度、费用、团队用量一眼看懂，用多少付多少。'
    },
    // 优势对比
    comparison: {
      title: '为什么选择我们？',
  ...
  aiStudio: {
    title: '对话生图',
    description: '在站内直接体验 AI 对话与图片生成',
    pendingApi: '使用当前账户 API 密钥发起对话与生图',
    entry: {
      badge: '轻量 AI 助手',
      title: '想聊天或生成图片，直接开启一个新窗口',
      description: '对话和生图使用同一个输入框；进入后可随时在底部切换模式，也可以返回这里重新开始。',
      chatTitle: '普通对话',
      chatDescription: '适合问答、翻译、写作、代码解释和提示词整理。',
      imageTitle: '生成图片',
      imageDescription: '适合把一句想法变成图片方向、海报草图或视觉参考。',
      startChat: '开始对话',
      startImage: '开始生图',
      windowTitle: '选择一个模式开始',
      windowSubtitle: '左侧管理历史与参数，右侧专注对话',
    },
    conversation: {
      back: '返回',
      chatGreeting: '今天想聊点什么？',
      imageGreeting: '描述你想生成的图片',
      chatHint: '像使用主流 AI 应用一样输入即可，回复会实时流式显示。',
      imageHint: '切到生图模式后，输入框仍是同一个，只是会按图片生成处理。',
      footerHint: '对话和生图将复用当前账户余额、分组权限和计费规则。',
    },
    input: {
      chatPlaceholder: '输入消息，按 Enter 发送...',
      imagePlaceholder: '描述你要生成的图片，按 Enter 提交...',
      upload: '上传文件',
      attachmentOnlyMessage: '已上传附件，请结合附件处理。',
    },
    stats: {
      balance: '账户余额',
      chatModel: '对话模型',
    ...
    settings: {
      title: '对话设置',
      description: '选择 API 密钥、模型与常用参数，请求会复用该密钥的分组权限和可用模型。',
      historyStorageTitle: '历史保存',
      historyStorageHint: '服务器数据库默认只保存 2 周历史；需要长期留存时，可以把当前会话保存到本浏览器本地存储。',
      saveLocalHistory: '保存到本地',
      autoSaveLocalHistory: '自动保存到本地',
      autoSaveLocalHistoryHint: '开启后，当前浏览器会自动保存历史对话，不再依赖服务端 14 天历史保留。',
      autoSaveEnabled: '已开启自动保存到本地',
      localHistoryRetention: '自动删除本地对话',
      localHistoryRetentionHint: '设置天数后，超过该天数的本地保存对话会自动删除；不设置则不自动删除。',
      noRetentionLimit: '不设置',
      daysAfter: '天后删除',
      localHistoryCount: '当前浏览器已长期保存 {count} 个会话',
      localHistorySaved: '当前会话已保存到本地浏览器',
      apiKey: 'API 密钥',
      apiKeyPlaceholder: '暂无 API 密钥',
      noAPIKey: '未选择 API 密钥',
      noAPIKeyMessage: '暂无 API 密钥，需要先前往“API 密钥”菜单创建并绑定 OpenAI 分组 API。是否现在前往？',
      noUsableAPIKey: '暂无可用于 AI Studio 的 API 密钥',
      noUsableAPIKeyHint: '已有 API 密钥，但暂不可用于 AI Studio：{reason}',
      noUsableAPIKeyMessage: '已有 API 密钥，但没有可用于 AI Studio 的 OpenAI 分组密钥：{reason}。是否前往“API 密钥”菜单调整？',
      selectedKeyUnavailable: '当前密钥不可用：{reason}',
      unavailableKey: '不可用',
      unavailableKeySummary: '{name} 不可用',
      noGroup: '未绑定分组',
      goManageAPIKey: '前往管理 API 密钥',
      goCreateAPIKey: '前往创建 API 密钥',
      modelUnavailable: '当前密钥无可用模型',
      apiKeyUnavailableReasons: {
        disabled: 'API 密钥未启用',
        expired: 'API 密钥已过期',
        quotaExhausted: 'API 密钥额度已用尽',
        missingGroup: '未绑定分组',
        nonOpenAIGroup: '当前仅支持 OpenAI 分组',
        groupDisabled: '绑定分组未启用',
        unknown: '暂不可用',
      },
      group: '分组',
      temperature: '温度',
      creativity: '创意强度',
      temperatureHint: '调低更稳定，调高更发散、更有创意。',
      moreParameters: '更多参数',
      chatModel: '对话模型',
      imageModel: '生图模型',
      maxTokens: '最大 Token',
      thinkingMode: '思考模式',
      thinkingModeHint: '开启后请求 reasoning summary，适合复杂问题；不支持的模型可能会报错。',
      ...
      qualityHd: '高清',
      systemPrompt: '系统提示词',
      systemPromptPlaceholder: '例如：你是一个简洁、准确的站内 AI 助手。',
      saved: '对话设置已保存',
    },
    groups: {
      default: '默认分组',
    ...
    actions: {
      ...
      delete: '删除',
      deleteConfirm: '确定删除这条历史对话吗？',
    },
    sample: {
      assistant: '你好，我可以帮你写作、分析问题、整理代码思路，也可以把想法延展成生图提示词。',
      user: '帮我把“夏日会员活动”整理成一段适合生成海报的提示词。',
      reply: '建议提示词：夏日会员专属活动海报，清爽橙白色调，玻璃杯冰饮、阳光纹理、简洁中文标题，电商运营风格，留出按钮与价格信息区域。',
      imageUser: '生成两张橙白配色的会员活动海报方向。',
      imageReply: '已为你生成 2 张预览方向。真实接口接入后，这里会展示上游返回的图片结果。',
      pendingReply: '收到。真实接口接入后，这里会返回模型的流式回复。',
    },
    chat: {
      title: 'AI 对话',
      subtitle: '适合日常问答、文案、代码和提示词整理',
      copy: '复制',
      edit: '编辑',
      resend: '重新发送',
      editPlaceholder: '编辑这条消息后重新发送...',
      waiting: 'Thinking',
      thinking: '思考中',
      thought: '思考过程',
    },
    image: {
      title: 'AI 生图',
      subtitle: '把想法快速变成可预览的图片方向',
      completed: '图片生成完成',
    },
    errors: {
      loadHistoryFailed: '加载服务端历史失败',
      loadConversationFailed: '加载对话失败',
      renameFailed: '重命名失败',
      deleteFailed: '删除失败',
      updateFailed: '更新失败',
      loadKeyOptionsFailed: '加载 API 密钥失败',
      runFailed: '请求失败',
      emptyResponse: '请求结束但没有返回内容，请检查当前 API 密钥绑定分组是否有可用账号。',
    },
  },
  ...
  admin: {
    ...
    settings: {
      ...
      payment: {
        ...
        displayMode: '支付展示模式',
        displayModeHint: '三选一：不启动、启用支付或仅展示套餐。',
        modeOff: '不启动',
        modePayment: '启用支付',
        modePlans: '展示套餐',
        modePlansHint: '用户可以浏览订阅套餐，但不会显示支付方式、余额充值和创建订单入口。',
  ...
  payment: {
    ...
    tabSubscribe: '订阅',
    noPlans: '暂无可用订阅套餐',
    externalSubscribe: '前往订阅',
    externalSubscribeContinue: '继续前往订阅',
    externalSubscribeClose: '稍后再说',
    externalSubscribeDialogFallbackTitle: '请先阅读订阅说明',
    ...
    admin: {
      ...
      sortOrder: '排序',
      forSale: '上架状态',
      externalSubscribe: '外链订阅',
      externalSubscribeHint: '仅展示套餐模式下，开启后用户可点击按钮跳转外链，或先查看一段主题弹窗说明。',
      externalSubscribeTargetType: '订阅方式',
      externalSubscribeTargetUrl: '跳转外链',
      externalSubscribeTargetDialog: '弹窗说明',
      externalSubscribeUrl: '订阅外链',
      externalSubscribeUrlPlaceholder: 'https://example.com/subscribe',
      externalSubscribeUrlRequired: '请先填写订阅外链',
      externalSubscribeDialogText: '订阅弹窗文本',
      externalSubscribeDialogTextHint: '填写后，用户点击“前往订阅”会先看到这段主题弹窗说明；若同时填写了外链，可在弹窗中继续跳转。',
      externalSubscribeDialogTextPlaceholder: '例如：请先联系客服开通，或加入群后按指引完成订阅。',
      externalSubscribeDialogTextRequired: '请先填写订阅弹窗文本',
      externalSubscribeTargetRequired: '请先填写订阅外链或弹窗文本',
      onSale: '上架',
      offSale: '下架',
      group: '分组',
      groupId: '分组 ID',
