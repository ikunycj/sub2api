...
type paymentOrderStaticLoadBalancer struct {
	selection *payment.InstanceSelection
}

func (lb *paymentOrderStaticLoadBalancer) GetInstanceConfig(context.Context, int64) (map[string]string, error) {
	return map[string]string{}, nil
}

func (lb *paymentOrderStaticLoadBalancer) SelectInstance(context.Context, string, payment.PaymentType, payment.Strategy, float64) (*payment.InstanceSelection, error) {
	if lb.selection != nil {
		return lb.selection, nil
	}
	return &payment.InstanceSelection{ProviderKey: payment.TypeAlipay}, nil
}
